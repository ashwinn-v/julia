...
a::LatticeElement ⊔ b::LatticeElement = begin
                    ...
                    if type_nfields == constant(b_nfields)::Int
                        if type_nfields ≠ 0
                            ...
                            for i = 1:type_nfields
                                ity = getfield_tfunc(a, Const(i)) ⊔ getfield_tfunc(b, Const(i))
                                fields[i] = ity
    ...
    mtyp = _typemerge(widenconst(a), widenconst(b))
...
@inline function typemerge(@nospecialize(a::Type), @nospecialize(b::Type))
    a === Bottom && return b
    b === Bottom && return a
    asub = a <: b
    asub && issimpleenoughtype(b) && return b
    ...
    asub && bsub && return a
    bsub && issimpleenoughtype(a) && return a
    return _typemerge(a, b)
end

@inline function _typemerge(@nospecialize(typea::Type), @nospecialize(typeb::Type))
